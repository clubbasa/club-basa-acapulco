...
export default function Home() {
  ...
  const share = async () => {
    const data = { title: 'Club BASA Acapulco', text: 'Mira el menú de Club BASA 👇', url: location.href };
  ...
  return <>
    <header className="nav"><div className="container navin">
      <a className="logo" href="#inicio" aria-label="Club BASA Acapulco"><span className="logoBlack">CLUB</span><span>BASA</span><small>ACAPULCO</small></a>
    ...
    <main id="inicio">
      ...
      <section id="menu"><Reveal><div className="container"><div className="sectionHead"><h2>Catálogo interactivo</h2><p>Productos y precios administrados desde Firestore. Si Firebase no está disponible, se muestra un catálogo de respaldo.</p></div>
        ...
        {items.length > 0 && <div className="cart"><div><strong>{items.reduce((sum, item) => sum + item.qty, 0)} productos</strong><br/><span>${total} + envío por confirmar</span></div><button className="btn" onClick={() => { track('whatsapp_order', { value: total }); location.href = waLink(buildOrder(items)); }}>Enviar pedido por WhatsApp</button></div>}
      </div></Reveal></section>
      ...
      <section><Reveal><div className="container"><div className="sectionHead"><h2>Comparte Club BASA</h2><p>Envíale la página a quien siempre pregunta “¿dónde compraste eso?”</p></div><div className="socials"><button className="social" onClick={share}>📤 Compartir</button><a className="social" href={`https://wa.me/?text=${encodeURIComponent('Mira el menú de Club BASA Acapulco: ' + location.href)}`}>WhatsApp</a><a className="social" href="https://www.facebook.com/sharer/sharer.php" target="_blank" rel="noreferrer">Facebook</a><a className="social" href="https://twitter.com/intent/tweet" target="_blank" rel="noreferrer">X</a></div></div></Reveal></section>
      ...
      <section id="contacto"><Reveal><div className="container contact"><div><div className="sectionHead"><h2>¿Quieres recibir promociones?</h2><p>Regístrate para acceder a promociones especiales y novedades.</p></div><button className="btn primary" onClick={() => location.href = '/login'}>Crear mi cuenta</button></div><ContactForm/></div></Reveal></section>
    </main>
