...
import { getCatalog, removeCategory, removeProduct, saveCategory, saveProduct, seedCatalog, type CatalogCategory, type CatalogProduct } from '@/lib/catalog';
...
export default function Admin() {
  ...
  const editCategory = (item: CatalogCategory) => {
    setCategory(item);
    requestAnimationFrame(() => {
  ...
  const initialize = async () => {
    await seedCatalog();
    await load();
    setMessage('Catálogo inicial sincronizado con Firestore.');
  };
  ...
  return <main className="container" style={{ padding: '50px 0 90px' }}>
    ...
    <section style={{ padding: '25px 0' }}>
      <div className="card"><h2>Primera configuración</h2><p>Si Firestore está vacío, carga los productos actuales como punto de partida. Después podrás editarlos desde este panel.</p><button type="button" className="btn primary" onClick={initialize}>Sincronizar catálogo inicial</button></div>
    </section>

    <section style={{ padding: '25px 0' }}>
      <div className="sectionHead"><h2>Productos</h2><p>{products.length} productos en Firestore.</p></div>
      <div className="grid3">
        {products.map((item) => <div className="card" key={item.id}>
          <strong>{item.name}</strong><p>{item.category} · ${item.price}</p><small>{item.active ? 'Activo' : 'Inactivo'}</small>
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button type="button" className="btn secondary" onClick={() => editProduct(item)}>Editar</button>
            <button type="button" className="btn secondary" onClick={async () => { await removeProduct(item.id); await load(); }}>Eliminar</button>
          </div>
        </div>)}
      </div>
    </section>

    <section id="product-editor" style={{ padding: '25px 0', scrollMarginTop: 24 }}>
      <div className="card">
        ...
        <div className="field"><label>Descripción</label><textarea value={product.description} onChange={(e) => setProduct({ ...product, description: e.target.value })} rows={3}/></div>
        <div className="field"><label>URL de imagen opcional</label><input value={product.image || ''} onChange={(e) => setProduct({ ...product, image: e.target.value })} placeholder="https://..." /></div>
        <button type="button" className="btn primary" onClick={saveProductForm}>Guardar producto</button>
      </div>
    </section>

    <section style={{ padding: '25px 0' }}>
      <div className="sectionHead"><h2>Categorías</h2><p>{categories.length} categorías en Firestore.</p></div>
      <div className="grid3">
        {categories.map((item) => <div className="card" key={item.id}>
          <strong>{item.name}</strong><p>{item.slug}</p>
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" className="btn secondary" onClick={() => editCategory(item)}>Editar</button>
            <button type="button" className="btn secondary" onClick={async () => { await removeCategory(item.id); await load(); }}>Eliminar</button>
          </div>
        </div>)}
      </div>
      <div id="category-editor" className="card" style={{ marginTop: 18, scrollMarginTop: 24 }}>
        <h3>{category.id ? 'Editar categoría' : 'Nueva categoría'}</h3>
        <div className="grid3">
          <div className="field"><label>ID</label><input value={category.id} onChange={(e) => setCategory({ ...category, id: e.target.value.trim().toLowerCase().replace(/\s+/g, '-') })}/></div>
          <div className="field"><label>Nombre</label><input value={category.name} onChange={(e) => setCategory({ ...category, name: e.target.value })}/></div>
          <div className="field"><label>Orden</label><input type="number" value={category.sortOrder} onChange={(e) => setCategory({ ...category, sortOrder: Number(e.target.value) })}/></div>
        </div>
        <button type="button" className="btn primary" onClick={saveCategoryForm}>Guardar categoría</button>
      </div>
    </section>
